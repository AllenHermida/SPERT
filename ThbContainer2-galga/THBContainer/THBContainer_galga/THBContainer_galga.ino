...
#define WIFI_AP "Ciro"
#define WIFI_PASSWORD "agusfc98"


//  configuración datos thingsboard
#define NODE_NAME "SPERT2"   //nombre que le pusieron al dispositivo cuando lo crearon
#define NODE_TOKEN "Y2xoKROsuXfmjFQgQ42B"   //Token que genera Thingboard para dispositivo cuando lo crearon
...
int dispenserOpenPos = 10;
int dispenserClosePos = 55;
...
unsigned int serverDispensingTime = 3350;
unsigned int halfCupDispensingTime = 1657;
unsigned int fullCupDispensingTime = 3350;
...
void processRequestAndReply(char *message, const char* topic)
{
  ...
  String methodName = doc["method2"];
  if (methodName.equals("setDispenseValue") &&  serverLockMode == false ) { // use for dispensing from server panel
    bool response  = doc["params2"];
    serverDispenseButtonState = response;

  }  else if (methodName.equals("setLockValue2")) { // use for locking dispenser from server
    bool response  = doc["params2"];
    serverLockMode = response;
    sendoLockStateToServer(serverLockMode);
  } else if (methodName.equals("empty2") &&  serverLockMode == false) { // notify that dispenser is empty acording to values calculated on server
    bool response  = doc["params2"];
    ...
    fullCupDispensingTime = doc["params2"]["elapseTaza"];
    halfCupDispensingTime = doc["params2"]["elapseTazaMedia"];
...
void sendoLockStateToServer(bool serverLockMode)
{
   ...
   //Update card
    ...
    doc["LockMessage2"] = serverLockMode ? "Bloqueado"  :"Habilitado" ;
...
void sendDispenseStateToServer(bool response)
{
   ...
   // serverDispenseButtonState = response;
    ...
    //Update card
    doc["dispensingState2"] = response ? "Dispensando"  :"Dispensado" ;
...
*/
    ...
    responseDoc["method2"] = "setDispenseValue";
    responseDoc["params2"] = response ? true  :false ;
    client.publish(attributesTopic, attributes);
...
void sendEmptyStateToServer(bool response)
{
   ...
   StaticJsonDocument<capacity> doc;
    ...
    doc["emptyState2"] = dispenserEmpty ? "Vacio"  :"Lleno" ;
